...
export interface TypeDefinition {
  ...
  // Note: /type/get/:id currently only returns entity fields, not location
  // TODO: This is incomplete - backend should return location too?
  declaration?: any;
  deprecated?: any;
  description?: string;
  id?: string;
...
class DarkPackagesApi {
  ...
  private getTypeNameFromDefinition(typeDef: TypeDefinition): string {
    if (typeDef.location?.name) {
      return typeDef.location.name;
    }
    // /type/get/:id endpoint doesn't return location currently
    return "Unknown";
  }
